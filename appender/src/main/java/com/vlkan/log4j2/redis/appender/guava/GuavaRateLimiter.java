...
import static com.vlkan.log4j2.redis.appender.guava.GuavaPreconditions.checkArgument;
import static com.vlkan.log4j2.redis.appender.guava.GuavaPreconditions.checkNotNull;
import static java.lang.Math.max;
import static java.util.concurrent.TimeUnit.MICROSECONDS;
import static java.util.concurrent.TimeUnit.SECONDS;

/**
 * A rate limiter. Conceptually, a rate limiter distributes permits at a configurable rate. Once
...
public abstract class GuavaRateLimiter {
  /**
  ...
  private static GuavaRateLimiter create(double permitsPerSecond, SleepingStopwatch stopwatch) {
    GuavaRateLimiter rateLimiter = new GuavaSmoothRateLimiter.SmoothBursty(stopwatch, 1.0 /* maxBurstSeconds */);
   ...
   */
  public static GuavaRateLimiter create(double permitsPerSecond, long warmupPeriod, TimeUnit unit) {
    checkArgument(warmupPeriod >= 0, "warmupPeriod must not be negative: %s", warmupPeriod);
    return create(
        permitsPerSecond, warmupPeriod, unit, 3.0, SleepingStopwatch.createFromSystemTimer());
  }

  private static GuavaRateLimiter create(
          double permitsPerSecond,
          long warmupPeriod,
          TimeUnit unit,
          double coldFactor,
          SleepingStopwatch stopwatch) {
    GuavaRateLimiter rateLimiter = new GuavaSmoothRateLimiter.SmoothWarmingUp(stopwatch, warmupPeriod, unit, coldFactor);
    rateLimiter.setRate(permitsPerSecond);
    return rateLimiter;
  }
  ...
  abstract static class SleepingStopwatch {
    ...
    static SleepingStopwatch createFromSystemTimer() {
      return new SleepingStopwatch() {
        final GuavaStopwatch stopwatch = GuavaStopwatch.createStarted();
        ...
        protected void sleepMicrosUninterruptibly(long micros) {
          if (micros > 0) {
            GuavaUninterruptibles.sleepUninterruptibly(micros, MICROSECONDS);
          }
        }
      };
    }
